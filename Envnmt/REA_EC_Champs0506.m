...
[Firesting.CompleteDataset]=uploading_firesting_2020('/Users/clement/Documents/Polytechnique/Cours/3A/Stage/Matlab/wetransfer-342365-2/Champs/20210605/20210605-Champs3.txt'); %uploading file created by oxymeter for the segment at 4Hz
Firesting.rawOxygen = Firesting.CompleteDataset{:,19};
Firesting.rawOxygenUP = Firesting.CompleteDataset{:,20};
Firesting.rawOxygenDOWN = Firesting.CompleteDataset{:,21};

[Firesting1]=uploading_firesting_2020('/Users/clement/Documents/Polytechnique/Cours/3A/Stage/Matlab/wetransfer-342365-2/Champs/20210605/20210605-Champs3.txt'); %uploading file created by oxymeter for the segment
...
[Firesting_initiation]=uploading_firesting_2020('/Users/clement/Documents/Polytechnique/Cours/3A/Stage/Matlab/wetransfer-342365-2/Champs/20210605/20210605-Champs2.txt'); %uploading file created by oxymeter for the segment
Firesting_initiation(1:32,:)=[];

probeCalibration.temp0 = 25;
  probeCalibration.temp100 = 25;
  probeCalibration.pressure = 1000;
  probeCalibration.humidity = 100;
  probeCalibration.dphi0 = 51.718;
  probeCalibration.dphi100 = 21.095;

  probeCalibration.UP.temp0 = 25;
  probeCalibration.UP.temp100 = 25;
  ...
  probeCalibration.DOWN.temp0 =  25;
  probeCalibration.DOWN.temp100 =  25;
  probeCalibration.DOWN.pressure = 1000;
...
filename = '/Users/clement/Documents/Polytechnique/Cours/3A/Stage/Matlab/wetransfer-342365-2/Champs/20210605/Champs04.dat';
...
filename = '/Users/clement/Documents/Polytechnique/Cours/3A/Stage/Matlab/wetransfer-342365-2/Champs/20210605/Champs04.sen';
...
% Oxygenhour1 = datetime(datetime(dayString+ ' '+string(rudelaloge2{1:end,2})), 'Format','HH:mm:ss');
% startLoge02 = max(tLoge2Hour(1), Oxygenhour1(4));
% OxiStartLoge02 = knnsearch(datenum(Oxygenhour1),datenum(startLoge02));
% endLoge02 = min(tLoge2Hour(end), Oxygenhour1(end-3));
% OxiEndLoge02 = knnsearch(datenum(Oxygenhour1),datenum(endLoge02))-1;
...
%     VeliStartLoge02 = seconds(Oxygenhour1(4) - tLoge2Hour(1))*8;
% end
% 
...
%   timestepOxygen2 = Firesting_initiation{:,3}-Firesting_initiation{1,3}; %Time (s) selected from the beginning of the segt giving the mesurement time step from each oxygen mesurement 
% timestepVelocity1 = timingLoge1(VeliStartLoge01:VeliEndLoge01)-timingLoge1(VeliStartLoge01);
% oxygen4Hz1 = oxygen4Hz1(OxiStartLoge01:OxiEndLoge01);
% 
%  [time1,oxygeninterp1] = interpolation_field_loge(timestepVelocity1,oxygen4Hz1,timestepOxygen1);
...
temp_0 = [15, 16]; %initial and final temperature during the day
...
% tplot = timew(1+(iSegment-1)*iSegmentDuration:iSegment*iSegmentDuration);
timew = tLoge(VeliStartLoge01:iSegmentDuration*QuantityofSegts+VeliStartLoge01-1);
tplot = timew(1+(iSegment-1)*iSegmentDuration:iSegment*iSegmentDuration);plot(timew(1:length(WFluctuation)), WFluctuation);
xlabel('Time');
ylabel('m/s');
title('Segment'+string(iSegment));
...
Depl = deployment_parameter_Champs( iDeployment, Param );%selecting parameters for each segment
...
Automate.velocityInitiationFile = readtable(Depl.velocityfileinitiationpath);
...
iStarthourlast = find(Automate.velocityFile{:,1}==Automate.velocityFile{iStartrunningAvg,1}, 1, 'last');
iStartSgt = iStarthourlast - (floor((iStarthourlast-iStartrunningAvg+1)/8))*8+1;
...
DateStart = datetime(datetime(dayString+ ' '+string(datetime(Automate.velocityFile{iStarthourlast,1},'Format', 'HH:mm'))+':'+StartSgtSecond), 'Format', 'HH:mm:ss');
...
OxiStart = find(Oxygenhour1==DateStart, 1, 'first');
OxiEnd = find(Oxygenhour1==DateEnd, 1, 'last');
OxDurationi = OxiEnd - OxiStart +1;
stdz = std(Automate.velocityFile{iStartSgt:iEndSgt,10});

OxEC = oxygeninterp1(((OxiStart-1)*2)+1:OxiEnd*2);
OxUP = oxygeninterp1Up(((OxiStart-1)*2)+1:OxiEnd*2);
OxDW = oxygeninterp1Dw(((OxiStart-1)*2)+1:OxiEnd*2);
    
oxygenHourComp = datetime(datetime(dayString+ ' '+string(Firesting_initiation{1:end,2})), 'Format','HH:mm:ss');
    ...
    Compensation.Ox.iStart = knnsearch(datenum(oxygenHourComp),datenum(Compensation.Firesting.Start));
    Compensation.Ox.iEnd = knnsearch(datenum(oxygenHourComp),datenum(Compensation.Firesting.End))-1;
...
Firesting.DOWN.timeInterp = time1(OxiStart:OxiEnd);
Firesting.DOWN.oxygenInterp = OxDW ;

Firesting.UP.timeInterp = time1(OxiStart:OxiEnd);
...
%despiking + conversion + interpolation

[oxygen4HziniUp, ~] = func_despike_phasespace3d(Firesting_initiation{:,20}/1000, 0, 2);
[oxygen4HziniDw, ~] = func_despike_phasespace3d(Firesting_initiation{:,21}/1000 ,0, 2);

timestepox = (Firesting_initiation{:,3}-Firesting_initiation{1,3})/100;
timestep8hz = (0:1/Param.ADVSAMPLINGFREQUENCY:timestepox(end))';

oxcompup = interpolation_field_loge(timestep8hz, oxygen4HziniUp, timestepox);
oxcompdw = interpolation_field_loge(timestep8hz, oxygen4HziniDw, timestepox);

for i = 1:length(oxcompup);

setCalibrationData(probeCalibration.DOWN.temp0,probeCalibration.DOWN.temp100, probeCalibration.DOWN.pressure, probeCalibration.DOWN.humidity, probeCalibration.DOWN.dphi0, probeCalibration.DOWN.dphi100);
F2 = calculateOxygen(oxcompup(i), 20, 1020, 0, 'Y');
oxcompup(i) = F2(5);

setCalibrationData(probeCalibration.UP.temp0,probeCalibration.UP.temp100, probeCalibration.UP.pressure, probeCalibration.UP.humidity, probeCalibration.UP.dphi0, probeCalibration.UP.dphi100);
F3 = calculateOxygen(oxcompdw(i), 20, 1020, 0, 'Y');
oxcompdw(i) = F3(5);

end

Compensation.Firesting.DOWN.oxygenCompensated = oxcompdw(((Compensation.Ox.iStart-1)*2)+1:Compensation.Ox.iEnd*2);
Compensation.Firesting.UP.oxygenCompensated = oxcompup(((Compensation.Ox.iStart-1)*2)+1:Compensation.Ox.iEnd*2);
...
meanW(16)= meanWcampaign;
stdW(16)= stdWcampaign;
FluxesShifted(16,2)= mean(FluxesShifted(:,2));
...
Para = {'Segment 1';'Segment 2';'Segment 3';'Segment 4';'Segment 5';'Segment 6';'Segment 7';'Segment 8';'Segment 9';'Segment 10';'Segment 11';'Segment 12';'Segment 13';'Segment 14';'Segment 15';'campaign'}
