...
for j =1:length(OxFluxInst_sgt)
    ...
    figure('Name','EC O2 Mean cumulated flux segment ' + string(iSegment),'NumberTitle','off');
    x = time(1:length(OxFluxMeanCumulated_sgt));
    y = OxFluxMeanCumulated_sgt;
    plot(x,y);
    ylabel('O_2 Mean cumulated Fluxes (mmol/m^2)');
    xlabel('Time');
    title({['EC O_2 Mean cumulated Fluxes'];['Segment ' + string(iSegment)]});
...
Depl = deployment_parameter_loge( iDeployment, Param );%selecting parameters for each segment
