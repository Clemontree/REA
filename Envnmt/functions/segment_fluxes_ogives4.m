function  [ecFluxHeat, ecFluxOxygen, ecFluxParticle, ... %reaFluxHeat, reaFluxOxygen, reaFluxParticle, ... 
    ecOgiveHeat, ecOgiveOxygen, ecOgiveParticle,... %reaOgiveHeat, reaOgiveOxygen, reaOgiveParticle, ...
        nFrequencyFFT]= ...
    segment_fluxes_ogives4(velocityZPrime_TimeShift, temperature, oxygenPrime_TimeShift, particle, reaBCoef, ...
    velocityZThreshold, MASSDENSITY, HEATCAPACITY, nFrequencyFFT, SAMPLINGFREQUENCY, RUNNINGAVERAGE)

% update June 2021
%This version of segment_flux_ogive doesn't compute the fluctuations of the
%concentration and vertical velocity.
%The fluctuations are already as input variables, computed previously by
%the segment_time_shift function
...
ecFluxHeat = 0;
    ecFluxOxygen    = 24*3600*              mean(velocityZPrime_TimeShift.*oxygenPrime_TimeShift);         % mmol/mÂ²/day
...
%     [CPSD_Tw, frequency]= cpsd(MASSDENSITY*HEATCAPACITY*temperaturePrime, velocityZPrime,[],[],nFrequencyFFT,SAMPLINGFREQUENCY);          % W mâ?»Â² Hzâ?»Â¹
    CPSD_O2w            = cpsd(24*3600*                 oxygenPrime_TimeShift,      velocityZPrime_TimeShift,[],[],nFrequencyFFT,SAMPLINGFREQUENCY);          % mmol mâ?»Â² Hzâ?»Â¹
