...
OxyiStartLoge01 = knnsearch(datenum(Oxygenhour1),datenum(startLoge01)); %finds the index for beginning of the measure in the time vector
...
OxyiEndLoge01 = knnsearch(datenum(Oxygenhour1),datenum(endLoge01))-1; %finds the index for end of the measure in the time vector
...
iEndSgt = iEndhour + floor((length(LogeBrut1{:,1})-iEndhour+1)/8)*8-1;
EndSgtSecond = floor((length(LogeBrut1{:,1})-iEndhour+1)/8);
...
OxDurationi = OxyiEndLogei - OxyiStartLogei +1;
stdz = std(LogeBrut1{iStartSgt:iEndSgt,10});

OxEC = oxygeninterp2(((OxyiStartLogei-1)*2)+1:OxyiEndLogei);
%OxUP
%OxDW
...
timestepVelocityi = timingLoge2(VeliStartLogei:VeliEndLogei)-timingLoge2(VeliStartLogei);
...
OxECIni = oxygeninterp2(((OxyiStartLogeIni-1)*2)+1:OxyiEndLogeIni);
%OxUP
%OxDW
